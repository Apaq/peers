...
package dk.apaq.peers.sip.transport;

import static dk.apaq.peers.sip.RFC3261.DEFAULT_SIP_VERSION;
import static dk.apaq.peers.sip.RFC3261.IPV4_TTL;
import static dk.apaq.peers.sip.RFC3261.PARAM_MADDR;
import static dk.apaq.peers.sip.RFC3261.PARAM_TTL;
import static dk.apaq.peers.sip.RFC3261.TRANSPORT_DEFAULT_PORT;
import static dk.apaq.peers.sip.RFC3261.TRANSPORT_PORT_SEP;
import static dk.apaq.peers.sip.RFC3261.TRANSPORT_SCTP;
import static dk.apaq.peers.sip.RFC3261.TRANSPORT_TCP;
import static dk.apaq.peers.sip.RFC3261.TRANSPORT_TLS_PORT;
import static dk.apaq.peers.sip.RFC3261.TRANSPORT_UDP;
import static dk.apaq.peers.sip.RFC3261.TRANSPORT_UDP_USUAL_MAX_SIZE;
import static dk.apaq.peers.sip.RFC3261.TRANSPORT_VIA_SEP;
import static dk.apaq.peers.sip.RFC3261.TRANSPORT_VIA_SEP2;
...
import dk.apaq.peers.sip.syntaxencoding.SipParser;
...
public class TransportManager {
    ...
    public MessageSender createClientTransport(SipRequest sipRequest,
        ...
        SipTransportConnection connection = new SipTransportConnection(config.getLocalInetAddress(), sipPort, inetAddress, port, transport);
    ...
    public void createServerTransport(String transportType, int port)
        ...
        SipTransportConnection conn = new SipTransportConnection(config.getLocalInetAddress(), port, null, SipTransportConnection.EMPTY_PORT, 
                transportType);
    ...
    public void sendResponse(SipResponse sipResponse) throws IOException {
        ...
        if (buf.indexOf(RFC3261.TRANSPORT_TCP) > -1) {
            ...
            LOG.error("no transport found in top via header, discarding response");
            return;
        }
    ...
    private MessageSender createMessageSender(SipTransportConnection conn)
        ...
        if (RFC3261.TRANSPORT_UDP.equalsIgnoreCase(conn.getTransport())) {
            ...
            if (datagramSocket == null) {
                LOG.debug("new DatagramSocket(" + conn.getLocalPort()
                ...
                LOG.info("added datagram socket " + conn);
            }
            socket = datagramSocket;
            messageSender = new UdpMessageSender(conn.getRemoteInetAddress(), conn.getRemotePort(), datagramSocket, config);
        } else {
            // TODO
...
//        }
    ...
    private MessageReceiver createMessageReceiver(SipTransportConnection conn,
        ...
        if (RFC3261.TRANSPORT_UDP.equalsIgnoreCase(conn.getTransport())) {
            ...
            messageReceiver = new UdpMessageReceiver(datagramSocket, transactionManager, this, config);
    ...
    private MessageReceiver createMessageReceiver(SipTransportConnection conn)
        ...
        if (RFC3261.TRANSPORT_UDP.equals(conn.getTransport())) {
            ...
            if (datagramSocket == null) {
                ...
                LOG.info("added datagram socket " + sipTransportConnection);
            }
            messageReceiver = new UdpMessageReceiver(datagramSocket, transactionManager, this, config);
            messageReceiver.setSipServerTransportUser(sipServerTransportUser);
        ...
        LOG.info("added " + sipTransportConnection + ": " + messageReceiver
